class GitGraphView {
  ...
  constructor(
    ...
    alterClass(
      ...
      config.branchLabelsAlignedToGraph,
    )
    ...
    remoteUrl.addEventListener('click', () => {
      showFormDialog(
        'New remote url',
        [
          {
    ...
    settingsBtn.innerHTML = svgIcons.cogs
  }
  ...
  public loadRepos(repos: GG.GitRepoSet, lastActiveRepo: string | null, loadRepo: string | null) {
    ...
    if (
      ...
      typeof repos[loadRepo] !== 'undefined'
    ) {
      this.currentRepo = loadRepo
      this.saveState()
      changedRepo = true
  ...
  public loadBranches(
    ...
    if (
      ...
      while (i < this.currentBranches.length) {
        if (
          branchOptions.indexOf(this.currentBranches[i]) === -1 &&
          globPatterns.indexOf(this.currentBranches[i]) === -1
        ) {
    ...
    for (let i = 0; i < this.config.customBranchGlobPatterns.length; i++) {
      options.push({
        name: 'Glob: ' + escapeHtml(this.config.customBranchGlobPatterns[i].name),
    ...
    if (
      ...
      arraysEqual(
        ...
        (a, b) =>
          ...
          arraysStrictlyEqual(a.tags, b.tags) &&
          arraysEqual(a.remotes, b.remotes, (a, b) => a.name === b.name && a.remote === b.remote) &&
          arraysStrictlyEqual(a.parentHashes, b.parentHashes),
      ) &&
        ...
        if (this.expandedCommit !== null && this.expandedCommit.srcElem !== null) {
          if (this.expandedCommit.compareWithHash === null) {
            ...
            if (
              ...
              (this.expandedCommit.hash === UNCOMMITTED ||
  ...
  private renderGraph() {
    ...
    let expandedCommit =
      this.config.commitDetailsViewLocation === 'Inline' ? this.expandedCommit : null
    let headerHeight = colHeadersElem.clientHeight + 1,
      expandedCommitElem = expandedCommit !== null ? document.getElementById('commitDetails') : null
  ...
  private renderTable() {
    let colVisibility = this.getColumnVisibility(),
      ...
      widthsAtVertices = this.config.branchLabelsAlignedToGraph
    ...
    for (let i = 0; i < this.commits.length; i++) {
      ...
      if (this.config.combineLocalAndRemoteBranchLabels) {
        ...
        for (j = 0; j < this.commits[i].remotes.length; j++) {
          let branchName = this.commits[i].remotes[j].name.substr(
            this.commits[i].remotes[j].remote.length + 1,
          )
      ...
      html +=
        '<tr class="commit"' +
        (this.commits[i].hash !== UNCOMMITTED ? '' : ' id="uncommittedChanges"') +
        ' data-hash="' +
        this.commits[i].hash +
        '" data-id="' +
        i +
        ...
        vertexColours[i] +
        '">' +
        (this.config.branchLabelsAlignedToGraph
          ? '<td style="padding-left:' +
            ...
            '</td><td>' +
            commitDot
          : '<td></td><td>' + commitDot + refBranches) +
        '<span class="gitRefTags">' +
        refTags +
        '</span>' +
    ...
    if (this.expandedCommit !== null) {
      ...
      for (let i = 0; i < elems.length; i++) {
        if (
          this.expandedCommit.hash === elems[i].dataset.hash ||
          this.expandedCommit.compareWithHash === elems[i].dataset.hash
        ) {
          ...
          if (
            elem !== null &&
            (this.expandedCommit.compareWithHash === null || compareWithElem !== null)
          )
            break
        }
      }
        ...
        if (this.expandedCommit.compareWithHash === null) {
          ...
          if (
            ...
            this.showCommitComparison(
              this.expandedCommit.hash,
              this.expandedCommit.compareWithHash,
              this.expandedCommit.fileChanges,
              this.expandedCommit.fileTree,
              false,
            )
            if (
      ...
      if (sourceElem.id === 'uncommittedChanges') {
        ...
        menu = [
                  ...
                  (hash, index) => ({
                    name:
                      ...
                      (typeof this.commitLookup[hash] === 'number'
                        ? ': ' + escapeHtml(this.commits[this.commitLookup[hash]].message)
                        : ''),
          ...
          },
          {
            ...
            onClick: () => {
              showFormDialog(
                'Are you sure you want to rebase the current branch on commit <b><i>' +
                  abbrevCommit(hash) +
                  '</i></b>?',
                [
                  {
                  ...
                  {
                    type: 'checkbox',
                    name: 'Ignore Date (non-interactive rebase only)',
                    value: true,
                  },
                ],
                'Yes, rebase',
                (values) => {
                  ...
                  runAction(
                    ...
                    interactive ? 'Launching Interactive Rebase' : 'Rebasing on Commit',
                  )
                },
                sourceElem,
              )
    ...
    addListenerToClass('gitRef', 'contextmenu', (e: Event) => {
      ...
      if (sourceElem.classList.contains('tag')) {
        menu = [
          {
            ...
            onClick: () => {
              showConfirmationDialog(
                ...
                () => {
                  runAction(
                    { command: 'deleteTag', repo: this.currentRepo, tagName: refName },
                    'Deleting Tag',
                  )
                },
                null,
          ...
          menu.push({
            ...
            onClick: () => {
              if (this.gitRemotes.length === 1) {
                ...
                let options = this.gitRemotes.map((remote, index) => ({
                  name: escapeHtml(remote),
                  value: index.toString(),
                }))
                showSelectDialog(
                  'Are you sure you want to push the tag <b><i>' +
                    escapeHtml(refName) +
                    '</i></b>? Select the remote to push the tag to:',
                  '0',
                  options,
                  'Yes, push',
                  (remoteIndex) => {
                    runAction(
                      {
                        ...
                        repo: this.currentRepo,
                        tagName: refName,
                        remote: this.gitRemotes[parseInt(remoteIndex)],
                      },
                      'Pushing Tag',
                    )
                  },
        ...
        let isHead = sourceElem.classList.contains('head'),
          isRemoteCombinedWithHead = (<HTMLElement>e.target).className === 'gitRefHeadRemote'
        ...
        if (isHead) {
          ...
          if (this.gitBranchHead !== refName) {
            menu.push(
              ...
              },
              {
                ...
                onClick: () => {
                  showFormDialog(
                    'Are you sure you want to rebase the current branch on branch <b><i>' +
                      escapeHtml(refName) +
                      '</i></b>?',
                    [
                      {
                    ...
                    (values) => {
                      ...
                      runAction(
                        ...
                        interactive ? 'Launching Interactive Rebase' : 'Rebasing on Branch',
                      )
                    },
                    null,
                  )
                      ...
                      runAction(
                        {
                          ...
                          repo: this.currentRepo,
                          branchName: refName,
                          remote: this.gitRemotes[parseInt(values[0])],
                          setUpstream: values[1] === 'checked',
                        },
                        'Pushing Branch',
                      )
                    },
          ...
          let remote = unescapeHtml(
            (isRemoteCombinedWithHead ? <HTMLElement>e.target : sourceElem).dataset.remote!,
          )
          menu = [
            {
                    ...
                    runAction(
                      {
                        ...
                        repo: this.currentRepo,
                        branchName: refName.substr(remote.length + 1),
                        remote: remote,
                      },
    ...
    addListenerToClass('gitRef', 'dblclick', (e: Event) => {
      ...
      if (!sourceElem.classList.contains('tag')) {
        let refName = unescapeHtml(sourceElem.dataset.name!),
          isHead = sourceElem.classList.contains('head'),
          isRemoteCombinedWithHead = (<HTMLElement>e.target).className === 'gitRefHeadRemote'
        if (isHead && isRemoteCombinedWithHead) {
          refName = unescapeHtml((<HTMLElement>e.target).dataset.remote!) + '/' + refName
          isHead = false
  ...
  private renderUncommittedChanges() {
    let colVisibility = this.getColumnVisibility(),
      date = getCommitDate(this.commits[0].date)
    ...
    if (remote !== null) {
      showRefInputDialog(
        'Enter the name of the new branch you would like to create when checking out <b><i>' +
          escapeHtml(refName) +
          '</i></b>:',
        refName.substr(remote.length + 1),
        'Checkout Branch',
        (newBranch) => {
          runAction(
            {
      ...
      cols[0].style.padding =
        ...
        Math.round(
          (Math.max(this.graph.getWidth() + 16, 64) -
            (cols[0].offsetWidth - COLUMN_LEFT_RIGHT_PADDING)) /
            2,
        ) +
        'px'
    }
  ...
  private observeWebviewStyleChanges() {
    let fontFamily = getVSCodeStyle('--vscode-font-family'),
      editorFontFamily = getVSCodeStyle('--vscode-editor-font-family')
    new MutationObserver(() => {
      let ff = getVSCodeStyle('--vscode-font-family'),
        eff = getVSCodeStyle('--vscode-editor-font-family')
        ...
        if (e.key === 'r' && (e.ctrlKey || e.metaKey)) {
          ...
          if (e.key === 'Escape') {
            ...
            if (e.key === 'ArrowUp' && this.commitLookup[this.expandedCommit.hash] > 0) {
              ...
              e.key === 'ArrowDown' &&
              this.commitLookup[this.expandedCommit.hash] < this.commits.length - 1
            ) {
              hashIndex = this.commitLookup[this.expandedCommit.hash] + 1
            }
            if (hashIndex > -1) {
              e.preventDefault()
              let hash = this.commits[hashIndex].hash,
                elems = <HTMLCollectionOf<HTMLElement>>document.getElementsByClassName('commit')
              for (let i = 0; i < elems.length; i++) {
  ...
  public closeCommitDetails(saveAndRender: boolean) {
    if (this.expandedCommit !== null) {
      ...
      if (typeof this.expandedCommit.srcElem === 'object' && this.expandedCommit.srcElem !== null)
        this.expandedCommit.srcElem.classList.remove(CLASS_COMMIT_DETAILS_OPEN)
      if (
        typeof this.expandedCommit.compareWithSrcElem === 'object' &&
        this.expandedCommit.compareWithSrcElem !== null
      )
        this.expandedCommit.compareWithSrcElem.classList.remove(CLASS_COMPARE_COMMIT_OPEN)
      this.expandedCommit = null
      if (saveAndRender) {
        this.saveState()
  ...
  public showCommitComparison(
    ...
    if (
      ...
      this.expandedCommit.srcElem === null ||
      this.expandedCommit.compareWithSrcElem === null ||
      this.expandedCommit.hash !== commitHash ||
  ...
  private renderCommitDetailsView(refresh: boolean) {
    ...
    let elem = isDocked ? this.dockedCommitDetailsView : document.getElementById('commitDetails'),
      html = ''
    if (elem === null) {
    ...
    if (this.expandedCommit.loading) {
      ...
      html +=
        '</div><div id="commitDetailsFiles">' +
        generateGitFileTreeHtml(this.expandedCommit.fileTree!, this.expandedCommit.fileChanges!) +
        '</div>'
    }
    ...
    elem.innerHTML = isDocked
      ? html
      : '<td></td><td colspan="' + (this.getNumColumns() - 1) + '">' + html + '</td>'
      ...
      if (isDocked) {
        ...
        if (this.config.autoCenterCommitDetailsView) {
          ...
          this.viewElem.scroll(
            0,
            elemTop - 12 + (this.config.grid.expandY - this.viewElem.clientHeight) / 2,
          )
          ...
          elemTop + this.config.grid.expandY - this.viewElem.clientHeight + 8 >
    ...
    if (!this.expandedCommit.loading) {
      let commitDetailsFiles = document.getElementById('commitDetailsFiles')!,
        timeout: NodeJS.Timer | null = null
      commitDetailsFiles.scroll(0, this.expandedCommit.fileChangesScrollTop)
      commitDetailsFiles.addEventListener('scroll', () => {
        if (this.expandedCommit !== null)
          this.expandedCommit.fileChangesScrollTop = commitDetailsFiles.scrollTop
        if (timeout !== null) clearTimeout(timeout)
        timeout = setTimeout(() => {
          this.saveState()
          timeout = null
  ...
  let gitGraph = new GitGraphView(
    ...
    viewState.loadRepo,
    {
      ...
      branchLabelsAlignedToGraph:
        viewState.refLabelAlignment === 'Branches (aligned to the graph) & Tags (on the right)',
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (msg.command) {
      ...
      case 'fetch':
        ...
        break
      case 'changeRemoteUrl':
        refreshOrDisplayError(msg.error, 'Unable to change the Remote url(s)')
        break
      case 'fetchAvatar':
      ...
      case 'rebaseOn':
        if (msg.error === null) {
          ...
          showErrorDialog(
            'Unable to Rebase current branch on ' + msg.type,
            msg.error,
            null,
            null,
            null,
...
function generateGitFileTreeHtml(folder: GitFolder, gitFiles: GG.GitFileChange[]) {
  ...
  for (i = 0; i < keys.length; i++) {
    if (folder.contents[keys[i]].type === 'folder') {
      ...
      diffPossible =
        gitFile.type === 'U' || (gitFile.additions !== null && gitFile.deletions !== null)
      html +=
        '<li class="gitFile ' +
        gitFile.type +
        ...
        gitFile.type +
        '" title="' +
        (diffPossible ? 'Click to view diff' : 'This is a binary file, unable to view diff.') +
...
function haveFilesChanged(
  oldFiles: GG.GitFileChange[] | null,
  newFiles: GG.GitFileChange[] | null,
) {
...
function runAction(msg: GG.RequestMessage, action: string) {
  showDialog(
    '<span id="actionRunning">' + svgIcons.loading + action + ' ...</span>',
    null,
    'Dismiss',
    null,
    null,
...
function showContextMenu(
  ...
  contextMenu.style.left =
    viewElem.scrollLeft +
    event.pageX +
    (event.pageX + bounds.width < viewElem.clientWidth ? -2 : 2 - bounds.width) +
    'px'
  contextMenu.style.top =
    viewElem.scrollTop +
    event.pageY +
    (event.pageY + bounds.height < viewElem.clientHeight ? -2 : 2 - bounds.height) +
    'px'
...
function showCheckedContextMenu(
  e: MouseEvent,
  ...
  for (let i = 0; i < items.length; i++) {
    items[i].title =
      ...
      (items[i].checked ? svgIcons.check : '') +
      '</span>' +
      items[i].title
  }
...
function showFormDialog(
  ...
  let textRefInput = -1,
    multiElement = inputs.length > 1
  let multiCheckbox = multiElement

  if (multiElement) {
    // If has multiple elements, then check if they are all checkboxes. If so, then the form is a checkbox multi
  ...
  showDialog(
    ...
    () => {
      if (
        dialog.className === CLASS_ACTIVE + ' noInput' ||
        dialog.className === CLASS_ACTIVE + ' inputInvalid'
      )
        return
      let values = []
      for (let i = 0; i < inputs.length; i++) {
